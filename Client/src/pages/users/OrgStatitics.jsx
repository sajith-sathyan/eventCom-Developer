...
function OrgStatitics() {
  ...
  useEffect(() => {
    try {
      const featchEvenetMedia = async () => {
        try {
          const eventMediaData = await axiosInstance.get(
            "/eventMedia"
          );
          console.log("eventMediaData---->", eventMediaData);
        } catch (err) {
          console.log(featchEvenetMedia, "featchEvenetMedia --> ERROR");
        }
      };
      featchEvenetMedia();
    } catch (err) {}
